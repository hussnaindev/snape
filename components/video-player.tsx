...
import {
  ...
  releaseActiveMedia,
  silenceOtherVideos,
} from '@/lib/active-media';
import { getCompletedDownloadForMedia, getDownloadObjectUrls } from '@/lib/downloads';
import { publishPlaybackProgress } from '@/lib/playback-progress';
import { cn } from '@/lib/utils';
import { getResumePosition } from '@/lib/watch-history';

const DOWNLOADED_PROVIDER = 'Downloaded';
...
interface Props {
  ...
  /**
   * Play only the on-device downloaded copy (no streaming fetch). Used by the
   * Downloads page. If no download exists, the player shows its no-sources state.
   */
  offlineOnly?: boolean;
}
...
export function prefetchStream(
  ...
  const inflight = startFullStreamFetch(key, `/api/stream/${type}/${tmdbId}${qs}`).finally(() =>
...
const qLabel = (s: StreamSource) =>
  s.type === 'hls' ? 'Auto' : s.quality ? `${s.quality}p` : 'Auto';
...
export function VideoPlayer({
  ...
  // Object URLs for an on-device downloaded copy (revoked on teardown).
  const localUrlsRef = useRef<{ video: string; subtitle: string | null } | null>(null);
  ...
  const [errorType, setErrorType] = useState<'no-sources' | 'playback-failed' | 'network' | null>(
    null,
  );
  ...
  const [menu, setMenu] = useState<
    null | 'settings' | 'quality' | 'speed' | 'server' | 'audio' | 'captions'
  >(null);
  ...
  useEffect(() => {
    ...
    // Revoke any object URLs from a previous load before resolving new ones.
    if (localUrlsRef.current) {
      URL.revokeObjectURL(localUrlsRef.current.video);
      if (localUrlsRef.current.subtitle) URL.revokeObjectURL(localUrlsRef.current.subtitle);
      localUrlsRef.current = null;
    }

    const run = async () => {
      // 1) On-device downloaded copy (offline source), if present.
      let localSource: StreamSource | null = null;
      let localSub: StreamSubtitle | null = null;
      try {
        const rec = await getCompletedDownloadForMedia(
          type === 'tv' ? 'series' : 'movie',
          tmdbId,
          season,
          episode,
        );
        if (rec && fetchGenRef.current === gen) {
          const urls = await getDownloadObjectUrls(rec.id);
          if (urls && fetchGenRef.current === gen) {
            localUrlsRef.current = urls;
            localSource = {
              type: 'mp4',
              url: urls.video,
              quality: Number.parseInt(rec.quality, 10) || null,
              dub: rec.lang || null,
              provider: DOWNLOADED_PROVIDER,
            };
            if (urls.subtitle) {
              localSub = { url: urls.subtitle, label: rec.subtitleLang ?? 'Subtitles', lang: null };
            }
          } else if (urls) {
            URL.revokeObjectURL(urls.video);
            if (urls.subtitle) URL.revokeObjectURL(urls.subtitle);
          }
        }
      } catch {
        // No download / IDB unavailable — fall through to streaming.
      }

      if (fetchGenRef.current !== gen) return;

      // Offline-only (Downloads page): play the local copy or show no-sources.
      if (offlineOnly) {
        if (localSource) {
          setSources([localSource]);
          setSubs(localSub ? [localSub] : []);
        } else {
          setStatus('error');
      ...
      // 2) Streaming sources, with the downloaded copy preferred when present.
      try {
        const json = await loadStreamData(key, `/api/stream/${type}/${tmdbId}${qs}`);
        if (fetchGenRef.current !== gen) return;
        const streamSources = json.ok && json.data ? json.data.sources : [];
        const streamSubs = json.ok && json.data ? json.data.subtitles : [];
        const allSources = [...(localSource ? [localSource] : []), ...streamSources];
        if (allSources.length === 0) {
          invalidateStreamCache(type, tmdbId, season, episode);
          setStatus('error');
          setErrorType('no-sources');
          if (IS_DEV)
            console.error('No sources available', { json, type, tmdbId, season, episode });
          return;
        }
        setSources(allSources);
        setSubs([...(localSub ? [localSub] : []), ...streamSubs]);
      } catch (err) {
        if (fetchGenRef.current !== gen) return;
        if (localSource) {
          // Streaming failed but we have the offline copy — play it.
          setSources([localSource]);
          setSubs(localSub ? [localSub] : []);
          return;
        }
  ...
  }, [type, tmdbId, season, episode, autoPlay, offlineOnly]);
  ...
  useEffect(() => {
    ...
    if (!best) return;
    // Prefer the on-device downloaded copy when available, so playback starts
    // from the offline file by default. Switching quality/server in the menu
    // falls back to streaming sources.
    const downloaded = sources.find((s) => s.provider === DOWNLOADED_PROVIDER);
    if (downloaded) {
      setLang(dubLabel(downloaded));
      setQuality(qLabel(downloaded));
      setServer(DOWNLOADED_PROVIDER);
      const enSub = subs.findIndex(
  ...
  useEffect(() => {
    ...
    const fallback = (error?: Error | null) => {
      ...
      if (next) {
        ...
        if (IS_DEV)
          console.error('All sources failed:', { failedSources: Array.from(failedRef.current) });
      }
    };
            ...
            >
              {playing ? (
                <svg
                  ...
                  aria-hidden="true"
                >
                  <path d="M12.75 4.5h-3A2.25 2.25 0 0 0 7.5 6.75v22.5a2.25 2.25 0 0 0 2.25 2.25h3A2.25 2.25 0 0 0 15 29.25V6.75a2.25 2.25 0 0 0-2.25-2.25Zm13.5 0h-3A2.25 2.25 0 0 0 21 6.75v22.5a2.25 2.25 0 0 0 2.25 2.25h3a2.25 2.25 0 0 0 2.25-2.25V6.75a2.25 2.25 0 0 0-2.25-2.25Z" />
                </svg>
              ) : (
                <svg
                  width="36"
                  ...
                  aria-hidden="true"
                >
                  <path d="M10.89 4.99A2.25 2.25 0 0 0 7.5 6.93v22.13a2.25 2.25 0 0 0 3.39 1.94l19-11.06a2.25 2.25 0 0 0 0-3.89l-19-11.06Z" />
                </svg>
              )}
            </button>
          )}
          ...
          >
            ...
            <div className="mt-1 flex flex-nowrap items-center gap-2 md:gap-4 text-white">
              ...
              <Ctrl
                onClick={() => skip(-10)}
                label="Back 10s"
                className={fullscreen ? 'inline-flex' : 'hidden md:inline-flex'}
              >
                <svg
                  width="22"
                  height="22"
              ...
              >
                <svg
                  ...
                  aria-hidden="true"
                >
                  <path d="m13 4 7 7-7 7" />
                  <path d="M20 11H9a5 5 0 0 0 0 10h1" />
                </svg>
              </Ctrl>
                ...
                <Ctrl onClick={toggleMute} label={muted ? 'Unmute' : 'Mute'}>
                  {muted || volume === 0 ? (
                    <svg width="24" height="24" viewBox="0 0 24 24" aria-hidden="true">
                      <path
                        fill="currentColor"
                        d="M11.6 2.08 11.48 2.14 3.91 6.68C3.02 7.21 2.28 7.97 1.77 8.87 1.26 9.77 1 10.79 1 11.83V12.16C1.07 13.52 1.37 14.46 1.87 15.29 2.38 16.12 3.08 16.81 3.91 17.31L11.48 21.85C11.63 21.94 11.8 21.99 11.98 21.99 12.16 22 12.33 21.95 12.49 21.87 12.64 21.78 12.77 21.65 12.86 21.5 12.95 21.35 13 21.17 13 21V3C12.99 2.83 12.95 2.67 12.87 2.52 12.8 2.37 12.68 2.25 12.54 2.16 12.41 2.07 12.25 2.01 12.08 2 11.92 1.98 11.75 2.01 11.6 2.08Z"
                      />
                      <path
                        fill="none"
                        stroke="currentColor"
              ...
              <span className="shrink-0 whitespace-nowrap text-[12px] md:text-[13px] tabular-nums text-white">
                {fmt(displayTime)}
                <span className="px-1 text-white/90"> / </span>
                {fmt(duration)}
              </span>
              ...
              <div className="ml-auto flex flex-nowrap items-center gap-2 md:gap-4 shrink-0">
                ...
                {subs.length > 0 && (
                  <Ctrl
                    onClick={() => {
                      setShowEpisodes(false);
                      setMenu(menu === 'captions' ? null : 'captions');
                    }}
                    label="Subtitles"
                    active={subIndex >= 0}
                  >
                ...
                >
                  <svg
                    ...
                    aria-hidden="true"
                  >
                    <path d="M25,18h-2v3h-3v2h5V18z M13,15h3v-2h-5v5h2V15z M27,9H9c-1.1,0-2,0.9-2,2v14c0,1.1,0.9,2,2,2h18c1.1,0,2-0.9,2-2V11C29,9.9,28.1,9,27,9z M27,25H9V11h18V25z" />
                  </svg>
                </Ctrl>
                ...
                <Ctrl onClick={toggleFullscreen} label="Fullscreen">
                  {fullscreen ? (
                    ...
                    <svg
                      ...
                      aria-hidden="true"
                    >
                      <path d="M10 3H3V10C3 10.26 3.1 10.51 3.29 10.7 3.48 10.89 3.73 11 4 11 4.26 11 4.51 10.89 4.7 10.7 4.89 10.51 5 10.26 5 10V6.41L9.29 10.7 9.36 10.77C9.56 10.92 9.8 11 10.04 10.99 10.29 10.98 10.52 10.87 10.7 10.7 10.87 10.52 10.98 10.29 10.99 10.04 11 9.8 10.92 9.56 10.77 9.36L10.7 9.29 6.41 5H10C10.26 5 10.51 4.89 10.7 4.7 10.89 4.51 11 4.26 11 4 11 3.73 10.89 3.48 10.7 3.29 10.51 3.1 10.26 3 10 3ZM20 13C19.73 13 19.48 13.1 19.29 13.29 19.1 13.48 19 13.73 19 14V17.58L14.7 13.29 14.63 13.22C14.43 13.07 14.19 12.99 13.95 13 13.7 13.01 13.47 13.12 13.29 13.29 13.12 13.47 13.01 13.7 13 13.95 12.99 14.19 13.07 14.43 13.22 14.63L13.29 14.7 17.58 19H14C13.73 19 13.48 19.1 13.29 19.29 13.1 19.48 13 19.73 13 20 13 20.26 13.1 20.51 13.29 20.7 13.48 20.89 13.73 21 14 21H21V14C21 13.73 20.89 13.48 20.7 13.29 20.51 13.1 20.26 13 20 13Z" />
                    </svg>
                  )}
                </Ctrl>
              </div>
            ...
            <div className="absolute bottom-[68px] md:bottom-[76px] right-3 md:right-5 min-w-44 max-h-[55vh] overflow-y-auto rounded-lg bg-black/95 border border-white/15 py-1 text-sm text-white shadow-2xl pointer-events-auto">
              {menu === 'settings' && (
                <>
                  ...
                  {servers.length > 1 && (
                    <Row label="Switch server" value={server} onClick={() => setMenu('server')} />
                  )}
...
function Row({ label, value, onClick }: { label: string; value: string; onClick: () => void }) {
  return (
    <button
      type="button"
      onClick={onClick}
      className="flex w-full items-center justify-between gap-6 px-3 py-2 hover:bg-white/10"
    >
      <span>{label}</span>
...
function Opt({ label, active, onClick }: { label: string; active: boolean; onClick: () => void }) {
  return (
    ...
    >
      <span
        className={cn('h-1.5 w-1.5 rounded-full', active ? 'bg-[#e50914]' : 'bg-transparent')}
      />
