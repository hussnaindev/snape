  ...
  return (
    ...
    >
      ...
      <div className="hidden sm:block aspect-video relative overflow-hidden">
        ...
        {/* Type chip top-left */}
        <span className="absolute top-1.5 left-2 lg:top-2 lg:left-3 z-10 inline-flex items-center text-[9px] lg:text-[9px] xl:text-[9px] font-semibold tracking-widest uppercase text-white/80 border border-white/40 rounded-full px-2 py-1 lg:px-2 lg:py-1 bg-black/60">
          {entry.type === 'movie' ? 'Film' : 'Series'}
        </span>

        {/* Rating chip top-right */}
        {entry.vote_average > 0 && (
          <span className="absolute top-1.5 right-2 lg:top-2 lg:right-3 z-10 inline-flex items-center gap-1 rounded-full border border-white/40 px-2 py-1 lg:px-2 lg:py-1 text-[9px] lg:text-[9px] xl:text-[9px] font-semibold tabular-nums text-white bg-black/60">
            ★ {entry.vote_average.toFixed(1)}
          </span>
        )}
  ...
  return (
    <section className="px-4 md:px-8 mt-6">
      ...
      <div className="flex gap-1.5 overflow-x-auto no-scrollbar snap-x snap-mandatory overscroll-x-contain scroll-pl-1 sm:scroll-pl-2 pt-3 sm:pt-4 pb-3 sm:pb-4 px-1 sm:px-2">
        {items.map((entry, idx) => (
          <ContinueWatchingCard key={`${entry.type}-${entry.id}`} entry={entry} prefetch={false} />
        ))}
      </div>
    </section>
  );
