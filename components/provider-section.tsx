...
import { type MediaItem, ProviderCard } from './provider-card';
...
export function ProviderSection({
  ...
  const backdropArtTranslate = {
    netflix: 'sm:translate-x-32',
    primevideo: 'sm:translate-x-32',
    paramountplus: 'sm:translate-x-32',
    disneyplus: ' sm:translate-x-0',
  } as Partial<Record<PreferredProviderKey, string>>;
  ...
  return (
    <section className="relative">
        ...
        <div className="absolute left-6 sm:left-12 top-[41%] -translate-y-1/2 z-20">
              ...
              <div className="flex flex-wrap items-center gap-3 sm:gap-4">
                ...
                {HERO_RT[providerKey] && (
                  <span
                    ...
                    style={{
                      fontSize: 11,
                      color: HERO_RT[providerKey].isFresh ? '#86efac' : '#fca5a5',
                    }}
                  >
                    <Image
          ...
          `}</style>
              ...
              <div className="flex gap-2 sm:gap-3">
                <Link
                  href={
                    HERO_TYPE[providerKey] === 'Movie'
                      ? `/movie/${HERO_ID[providerKey]}`
                      : `/series/${HERO_ID[providerKey]}`
                  }
      ...
      <ParallaxCarousel className="-mt-16 sm:-mt-20 relative z-30 flex gap-1.5 px-5 sm:px-10 overflow-x-auto no-scrollbar pb-3 sm:pb-4 pt-4 sm:pt-6">
        {items.map((item) => (
          <ProviderCard key={`${item.kind}-${item.id}`} item={item} />
        ))}
      </ParallaxCarousel>
    </section>
  );
