...
const badHits = [
  {
    title: 'In the Grey',
    postTitle: 'In the Grey[CAM]_1080P',
  ...
  },
  {
    ...
    postTitle: 'The Shawshank Redemption',
...
assert.equal(
  detectMovieboxLanguage({ title: 'Inception', postTitle: 'Inception[CAM]_1080P' }),
  'Original',
);
...
const inceptionItems = [
  ...
  {
    title: 'Inception',
    postTitle: 'Inception[CAM]_1080P',
    releaseDate: '2010-07-16',
    duration: 8880,
    imdbRatingValue: '8.8',
    hasResource: true,
...
assert.deepEqual(
  ...
  ['Original', 'Hindi', 'Tamil'],
);
assert.ok(
  !variants.some((v) => v.item.subjectId === 'C'),
  'must not group "Inception: The Cobol Job"',
);
...
assert.equal(
  pickBestMovieboxMatch(tmdbInception, remake),
  null,
  'contradicting-year remake must stay excluded',
);
